        ...
        for (b in bs) {
            ...
            for (r in rs) {
                ...
                sum_poncta_weighted <- sapply(
                    var_names,
                    function(v) sum(nb_poncta_weighted[names(nb_poncta_weighted) == v])
                )

                for (c in df_contour$cell_id) {
                ...
                tables[[i]] <- c(d, b, r, m, t(nb_cells), sum(nb_cells), as.matrix(nb_poncta_weighted), sum_poncta_weighted, sum(nb_cells > 0), stats)
            }
        }
    }
}
...
cols_poncta <- c(
    ...
    paste0("nb_poncta_tot", c("", "_weighted"))
)
...
colnames(list_tables[[2]]) <- c(keys[4:6], keys[8], paste0("nb_poncta_", seq(4)), "sum_poncta", cols_p_weight, cols_s_p_weight, "nb_cells", cols)
...
for (m in types) {
    for (d in diseases) {
        ...
        for (b in bs) {
            rs <- sort(unique(data[data$brain_id == b, ]$image_id))
            i <- i + 1
            condition <- c(keys[4:5], keys[8])
            df <- data[
                data$disease_grp == d &
                data$brain_id == b &
                data$modality == m, ]
            nb_poncta <- sum(list_tables[[2]][
                list_tables[[2]]$disease_grp == d &
                list_tables[[2]]$brain_id == b &
                list_tables[[2]]$modality == m, ]$nb_poncta_tot) # nb_poncta_ponderated
            tables[[i]] <- c(d, b, m, length(rs), nb_cells, calculate_stats(df))
        }
    }
}

list_tables[[3]] <- data.frame(Reduce(rbind, tables))
colnames(list_tables[[3]]) <- c(keys[4:5], keys[8], "nb_images", "nb_cells", cols)
...
colnames(list_tables[[4]]) <- c(keys[4], keys[8], "nb_brains", cols)
